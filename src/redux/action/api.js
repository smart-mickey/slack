export const baseURL = 'http://localhost:3001/api';
export const LOGIN = `${baseURL}/auth/login`;
export const REGISTER = `${baseURL}/auth/register`;
export const SEND_MESSAGE = `${baseURL}/chat/sendMessage`;
export const FETCH_CHATS = `${baseURL}/chat/fetchChatData`;
export const GET_USER_DATA = `${baseURL}/chat/getUserData`;
export const CREATE_WORKSPACE = `${baseURL}/workspace/createWorkSpace`;
export const CHECK_WORKSPACE = `${baseURL}/workspace/checkWorkSpace`;
export const GET_WORKSPACE = `${baseURL}/workspace/getWorkSpace`;

